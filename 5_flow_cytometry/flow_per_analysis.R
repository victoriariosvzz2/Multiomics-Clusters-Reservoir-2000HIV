#!/usr/bin/env Rscript
# =============================================================================
# Script: flow_per_analysis.R
# Project: Multi-Omics Clustering - 2000HIV Study
# Description: Flow cytometry immunophenotyping analysis (percentage of
#              parent population) comparing two MoCluster endotypes at a
#              time. For each of the complete cohort, discovery cohort, and
#              validation cohort separately: linear regression of each cell
#              population against cluster membership (adjusted for season,
#              age, time-to-lab, COVID vaccination, sex, genetic PC1),
#              summary heatmaps, and discovery-vs-validation 4-quadrant
#              consistency plots.
...
#
# NOTE: this comparison's original notebook (FlowPER_Analysis_MOC3_vs_MOC2)
# had `ref` set to the wrong group (AllHigh instead of AllLow), which would
# have flipped the sign of every effect estimate relative to what the
# filenames claimed. The group_a/group_b parameterization here fixes that —
# double check any results carried over from the original PER notebook run
# for this comparison specifically.
## Follows: flow_abs_analysis.R (same comparisons, absolute count data)
# =============================================================================
...
# --- Flow cytometry data (percentage of parent population) ---
df <- read_excel(file.path(data_dir, "omics/2000HIV_Flow_Cytometry/2000HIV_FLOW_PER_panel123merged_QCed_untransformed(raw)data_1434samples_356vars_Nov062023.xlsx"))
...
plot_regression_heatmaps <- function(results, scope_label) {
  ...
  for (ext in c("jpg", "pdf")) {
    ggsave(file.path(output_dir, paste0("flow_cytometry/FCM_PER_", comparison_label, "_", scope_label, "_AgeSexSeasonTimetolabPC1Covidvacc.", ext)),
  ...
  write.xlsx(sig_highlights, file.path(output_dir, paste0("flow_cytometry/FCM_PER_Results_", comparison_label, "_", scope_label, "_AgeSexSeasonTimetolabPC1Covidvacc.xlsx")))
  ...
  for (ext in c("tiff", "pdf")) {
    ggsave(file.path(output_dir, paste0("flow_cytometry/FCM_PER_Highlights_", comparison_label, "_", scope_label, "_AgeSexSeasonTimetolabPC1Covidvacc.", ext)),
...
write_xlsx(results_complete, file.path(output_dir, paste0("flow_cytometry/FCM_PER_", comparison_label, "_complete_AgeSexSeasonTimetolabPC1Covidvacc.xlsx")))
...
write_xlsx(results_discovery, file.path(output_dir, paste0("flow_cytometry/FCM_PER_", comparison_label, "_discovery_AgeSexSeasonTimetolabPC1Covidvacc.xlsx")))
...
write_xlsx(results_validation, file.path(output_dir, paste0("flow_cytometry/FCM_PER_", comparison_label, "_validation_AgeSexSeasonTimetolabPC1Covidvacc.xlsx")))
...
plot_4qp <- function(sig_basis = c("fdr", "pvalue"), marker_set = c("all", "broad")) {
  ...
  filename_base <- paste0("flow_cytometry/FCM_PER_4QP_", comparison_label, "_discovery_and_validation_AgeSexSeasonTimetolabPC1Covidvacc_", file_suffix)
  for (ext in c("tiff", "pdf")) {
...
# -----------------------------------------------------------------------------
# 5. Export Source Data for Reviewers
# -----------------------------------------------------------------------------
...
shared_output_path <- file.path(output_dir, "source_data/SourceData_4QuadrantPlots_FCM_PER.xlsx")
...
export_4qp_panel(results_discovery, results_validation,
                  paste0(comparison_label, "_PCT"), shared_output_path,
